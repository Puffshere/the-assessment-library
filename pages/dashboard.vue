...
export default {
  ...
  computed: {
    ...
    storyTokens() {
      const profile = this.$store.state.activeChildProfile;
      if (!profile) return 0;
      const completed = profile.completedAssessments
        ? profile.completedAssessments.length
        : 0;
      const written = profile.chaptersWritten || 0;
      return Math.max(0, completed - written);
    },
  },
  async mounted() {
    this.loading = true;
  ...
  methods: {
    async generateStory() {
      const profile = this.$store.state.activeChildProfile;
      if (!profile || this.isGeneratingStory) return;
      this.isGeneratingStory = true;
      try {
        await this.$axios.$post(`/api/story/${profile._id}/generate`);
        this.$store.commit("SET_ACTIVE_CHILD_PROFILE", {
          ...profile,
          chaptersWritten: (profile.chaptersWritten || 0) + 1,
        });
        this.$router.push(`/kids/story/${profile._id}`);
      } catch (err) {
        alert(
...
.dashboard {
  ...
  .story-token-btn {
    ...
    padding: 10px 28px;
    border-radius: 8px;
    background: linear-gradient(135deg, #0033c5, #00a8ff);
    color: #fff;
    border: none;
    font-family: $font-family;
    font-size: 14px;
