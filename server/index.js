...
async function start() {
  ...
  app.use(async (ctx, next) => {
    if (ctx.path === '/manifest.json') {
      if (ctx.hostname.includes('governmentassessments.com')) {
        ...
        ctx.body = require(path.join(__dirname, '../static/manifest-government.json'));
      } else {
        ctx.type = 'application/json';
        ctx.body = require(path.join(__dirname, '../static/manifest-assessments.json'));
      }
    } else {
      await next();
    }
  ...
  app.use(async (ctx, next) => {
    ...
    if (ctx.hostname.includes('governmentassessments.com')) {
      ctx.req.url = '/government-page'; // Modify the URL for Nuxt rendering
    }
    await next();
  });
